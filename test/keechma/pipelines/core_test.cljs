...
(deftest detach-1
  (let [{:keys [state*] :as context} (make-context)
        ...
        runtime   (start! context pipelines
                    #_{:watcher #(println (with-out-str (cljs.pprint/pprint %4)))})]
    (async done
      (go
        (is (nil? @state*))
...
(deftest changing-interpreter-state-3
  (let [{:keys [state*] :as context} (make-context)
    ...
    (async done
      (is (nil? @state*))
      (-> (invoke runtime :run 1)
        (p/then #(is (= [:shared 1 1] @state*)))
        (p/then #(invoke runtime :run 2))
        (p/then #(is (= [:shared 2 :resumed] @state*)))
        (p/then done)))))
...
(deftest changing-interpreter-state-4
  (let [{:keys [state*] :as context} (make-context)
    ...
    (async done
      (is (nil? @state*))
      (-> (invoke runtime :run 1)
        (p/then #(is (= [:shared 1 :nested 1] @state*)))
        (p/then #(invoke runtime :run 2))
        (p/then #(is (= [:shared 2 :nested :resumed] @state*)))
        (p/then done)))))
